...
spec = do
  ...
  describe "First'" $ do
    ...
    it "obeys right identity" $
      property (monoidRightIdentity :: First' S -> B)
    it "obeys left identity" $
      property (monoidLeftIdentity :: First' S -> B)
    it "for an Only and Nada" $
      (First' $ Only [1]) `mappend` First' Nada `shouldBe` (First' $ Only ([1] :: [Int]))
    it "for a Nada and Only" $
      First' Nada `mappend` (First' $ Only [1]) `shouldBe` (First' $ Only ([1] :: [Int]))
    it "for both Nada" $
      First' Nada `mappend` First' Nada `shouldBe` (First' $ (Nada :: Optional [Int]))
    it "for both Only" $
      (First' $ Only [1]) `mappend` (First' $ Only [2]) `shouldBe` (First' $ Only ([1] :: [Int]))
  describe "Trivial" $ do
    it "obeys associativity" $
      property (semigroupAssoc :: Trivial -> Trivial -> Trivial -> B)
    it "obeys right identity" $
      property (monoidRightIdentity :: Trivial -> B)
    it "obeys left identity" $
      property (monoidLeftIdentity :: Trivial -> B)
  describe "Identity" $ do
    ...
    it "obeys left identity" $
      property (monoidLeftIdentity :: Identity S -> B)
  describe "Two" $ do
    it "obeys associativity" $
      property (semigroupAssoc :: Two S S -> Two S S -> Two S S -> B)
    it "obeys right identity" $
      property (monoidRightIdentity :: Two S S -> B)
    it "obeys left identity" $
      property (monoidLeftIdentity :: Two S S -> B)
  describe "Three" $ do
    it "obeys associativity" $
      property (semigroupAssoc :: Three S S S -> Three S S S -> Three S S S -> B)
    it "obeys right identity" $
      property (monoidRightIdentity :: Three S S S -> B)
    it "obeys left identity" $
      property (monoidLeftIdentity :: Three S S S -> B)
  describe "Four" $ do
    it "obeys associativity" $
      property (semigroupAssoc :: Four S S S S -> Four S S S S -> Four S S S S -> B)
    it "obeys right identity" $
  ...
  describe "BoolConj" $ do
    ...
    it "obeys right identity" $
      property (monoidRightIdentity :: BoolConj -> B)
    it "obeys left identity" $
      property (monoidLeftIdentity :: BoolConj -> B)
    it "for True/True" $
  ...
  describe "BoolDisj" $ do
    ...
    it "obeys right identity" $
      property (monoidRightIdentity :: BoolDisj -> B)
    it "obeys left identity" $
      property (monoidLeftIdentity :: BoolDisj -> B)
    it "for True/True" $
  ...
  describe "Comp" $ do
    it "for inc <> sub with 0" $
      (unComp (compInc <> compSub) $ 0) `shouldBe` 0
    it "for inc <> sub with 1" $
      (unComp (compInc <> compSub) $ 1) `shouldBe` 1
    it "for inc <> inc with 1" $
      (unComp (compInc <> compInc) $ 1) `shouldBe` 3
    it "for sub <> inc with 1" $
      (unComp (compSub <> compInc) $ 1) `shouldBe` 1
    it "for inc <> mempty with 1" $
      (unComp (compInc <> mempty) $ 1) `shouldBe` 2
  describe "Validation" $ do
    it "obeys associativity" $
      property (semigroupAssoc :: Validation S S -> Validation S S -> Validation S S -> B)
    it "for Failure/Failure" $
      Failure "woot" <> Failure "blah" `shouldBe` (Failure "wootblah" :: Validation String Int)
    it "for Failure/Success" $
      Failure "woot" <> Success 2 `shouldBe` (Success 2 :: Validation String Int)
    it "for Success/Failure" $
      Success 1 <> Failure "woot" `shouldBe` (Success 1 :: Validation String Int)
  ...
  describe "Mem" $ do
    ...
    it "testMem" $
      runMem testMem 0 `shouldBe` ("hi", 1)
    it "rmleft" $
      runMem (testMem <> mempty) 0 `shouldBe` ("hi", 1)
    it "rmright" $
      runMem (mempty <> testMem) 0 `shouldBe` ("hi", 1)
\end{code}
