...
myWords string = firstWord : myWords restOfString
  where firstWord = takeWhile (/= ' ') string
...
splitOn c string = firstLine : splitOn c restOfString
  where firstLine = takeWhile (/= c) string
...
spec = do
  describe "tensDigit" $ do
    it "for 1" $
      tensDigit 1 `shouldBe` 0
    it "for 123" $
      tensDigit 123 `shouldBe` 2
    it "for 1234" $
      tensDigit 1234 `shouldBe` 3

  describe "hundredsDigit" $ do
    it "for 1" $
      hundredsDigit 1 `shouldBe` 0
    it "for 123" $
      hundredsDigit 123 `shouldBe` 1
    it "for 1234" $
      hundredsDigit 1234 `shouldBe` 2

  describe "foldBool with case" $ do
    it "with True" $
      foldBool1 1 2 True `shouldBe` 1
    it "with False" $
      foldBool1 1 2 False `shouldBe` 2

  describe "foldBool with guard" $ do
    it "with True" $
      foldBool2 1 2 True `shouldBe` 1
    it "with False" $
      foldBool2 1 2 False `shouldBe` 2

  describe "g" $ do
    it "works" $
      g (1 +) (2, 'a') `shouldBe` (3, 'a')

  describe "roundTrip" $ do
    it "works" $
      roundTrip 4 `shouldBe` id 4
  ...
  describe "lists" $ do
    it "tuples" $
      tuples `shouldBe` [ (1^2, 1^3), (1^2, 2^3), (1^2, 3^3), (1^2, 4^3), (1^2, 5^3)
                        , (2^2, 1^3), (2^2, 2^3), (2^2, 3^3), (2^2, 4^3), (2^2, 5^3)
                        , (3^2, 1^3), (3^2, 2^3), (3^2, 3^3), (3^2, 4^3), (3^2, 5^3)
                        , (4^2, 1^3), (4^2, 2^3), (4^2, 3^3), (4^2, 4^3), (4^2, 5^3)
                        , (5^2, 1^3), (5^2, 2^3), (5^2, 3^3), (5^2, 4^3), (5^2, 5^3) ]
    it "tuplesUnder50" $
      tuplesUnder50 `shouldBe` [ (1^2, 1^3), (1^2, 2^3), (1^2, 3^3)
                               , (2^2, 1^3), (2^2, 2^3), (2^2, 3^3)
                               , (3^2, 1^3), (3^2, 2^3), (3^2, 3^3)
                               , (4^2, 1^3), (4^2, 2^3), (4^2, 3^3)
                               , (5^2, 1^3), (5^2, 2^3), (5^2, 3^3) ]
    it "howManyTuplesUnder50" $
      howManyTuplesUnder50 `shouldBe` 15

  describe "filtering" $ do
    it "multiplesOfThree" $
      multiplesOfThree `shouldBe` [3, 6, 9, 12, 15, 18, 21, 24, 27, 30]
    it "numberOfMultiplesOfThree" $
      numberOfMultiplesOfThree `shouldBe` 10
    it "myFilter" $
      myFilter "the brown dog was a goof" `shouldBe` ["brown", "dog", "was", "goof"]
\end{code}
