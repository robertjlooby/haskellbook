...
cypher n (c:cs)
  ...
  where
    ...
    upperAOffset = ord 'A'
    shift aOffset =
      chr . (+ aOffset) . (flip mod 26) . (+ n) . (flip (-) aOffset) . ord
...
unCypher n (c:cs)
  ...
  where
    ...
    shift aOffset =
      chr .
      (+ aOffset) . (flip mod 26) . (flip (-) n) . (flip (-) aOffset) . ord
...
spec = do
  describe "myZip" $ do
    it "same length" $ myZip [1, 2] ['a', 'b'] `shouldBe` [(1, 'a'), (2, 'b')]
    it "first longer" $ myZip [1, 2] ['a'] `shouldBe` [(1, 'a')]
    it "second longer" $ myZip [1] ['a', 'b'] `shouldBe` [(1, 'a')]
  describe "myZipWith" $ do
    it "same length" $ myZipWith (+) [1, 2] [3, 4] `shouldBe` [4, 6]
    it "first longer" $ myZipWith (+) [1, 2] [3] `shouldBe` [4]
    it "second longer" $ myZipWith (+) [1] [3, 4] `shouldBe` [4]
  describe "myZip2" $ do
    it "same length" $ myZip2 [1, 2] ['a', 'b'] `shouldBe` [(1, 'a'), (2, 'b')]
    it "first longer" $ myZip2 [1, 2] ['a'] `shouldBe` [(1, 'a')]
    it "second longer" $ myZip2 [1] ['a', 'b'] `shouldBe` [(1, 'a')]
  describe "caps" $ do
    it "keeps the capitals" $ caps "HbEfLrLxO" `shouldBe` "HELLO"
  describe "capitalize" $ do
    it "capitalizes the first letter" $ capitalize "julie" `shouldBe` "Julie"
  describe "allCaps" $ do
    it "capitalizes all the letters" $ allCaps "woot" `shouldBe` "WOOT"
  ...
  describe "myReverse" $ do
    ...
    it "True" $ myReverse [1 .. 5] `shouldBe` [5, 4, 3, 2, 1]
  describe "squish" $ do
  ...
  describe "myMaximum" $ do
    it "gets the max" $ myMaximum [1, 53, 9001, 10] `shouldBe` 9001
  describe "myMinimum" $ do
    it "gets the min" $ myMinimum [1, 53, 9001, 10] `shouldBe` 1
\end{code}
